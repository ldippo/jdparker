...
$medicom_post_metabox = array(
    'youtube_link_url' => array(
        'title' => __('Video Link', 'medicom'),
        'applicableto' => 'post',
        'location' => 'normal',
    ...
    'audio_link_url' => array(
        ...
        'fields' => array(
            'a_url' => array(
                'title' => __('Soundcloud Widget Code:', 'medicom'),
                'type' => 'text',
...
add_action( 'admin_init', 'medicom_add_post_format_metabox' );
 
function medicom_add_post_format_metabox() {
    global $medicom_post_metabox;
 
    if ( !empty( $medicom_post_metabox ) ) {
        foreach ( $medicom_post_metabox as $id => $metabox ) {
            add_meta_box( $id, $metabox['title'], 'medicom_show_metaboxes', $metabox['applicableto'], $metabox['location'], $metabox['priority'], $id );
        }
    }
}
...
function medicom_show_metaboxes( $post, $args ) {
    global $medicom_post_metabox;
 
    $custom = get_post_custom( $post->ID );
    $fields = $tabs = $medicom_post_metabox[$args['id']]['fields'];
...
/* Save Metabox */
add_action( 'save_post', 'medicom_save_metaboxes' );
 
function medicom_save_metaboxes( $post_id ) {
    global $medicom_post_metabox;
    ...
    foreach ( $medicom_post_metabox as $id => $metabox ) {
        ...
        if ( $metabox['applicableto'] == $post_type ) {
            $fields = $medicom_post_metabox[$id]['fields'];
...
add_action( 'admin_print_scripts', 'medicom_display_metaboxes', 1000 );
function medicom_display_metaboxes() {
    global $medicom_post_metabox;
...
/******************************/
/* medicom Video Post Function */
/******************************/

function medicom_video( $height ) {
...
/*******************************/
/* medicom Audio Post Function */
/******************************/

function medicom_audio () {
    $audio_url ="";
...
/********************************/
/* medicom Gallery Post Function  */
/*******************************/
function medicom_gallery ($id) {
